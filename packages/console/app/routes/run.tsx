...
export function meta() {
  return [{ title: "Run a workflow — Cori Console" }];
}
...
export default function RunPage() {
  ...
  async function inspect(e: React.FormEvent) {
    ...
    setConsent(null);
    try {
      const pf = await apiGet<WorkflowPreflight>(
        `/api/workflow?source=${encodeURIComponent(source)}`,
      );
      setPreflight(pf);
      ...
      setParams(defaults);
      if (pf.consent_required) setConsent(pf.consent_required);
    } catch (e: unknown) {
  ...
  async function trustAndRetry() {
    ...
    setError(null);
    try {
      await apiPost("/api/trust", {
        host: consent.host,
        repo: consent.repo,
        subpath: consent.subpath,
        ref: consent.ref,
        sha: consent.sha,
        declared_capabilities: consent.declared_capabilities,
      });
      setConsent(null);
      // Re-inspect to refresh preflight (now ready).
  ...
  async function startRun(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await apiPost<TriggerResponse>("/api/runs", {
        source,
        params,
        dry_run: dryRun,
      });
      ...
      if (e instanceof ApiError && e.status === 409) {
        const body = e.body as { consent_required?: ConsentRequired };
        if (body.consent_required) setConsent(body.consent_required);
        else setError("consent required (no details supplied)");
      } else {
        setError(e instanceof Error ? e.message : String(e));
      }
    } finally {
          ...
          <form onSubmit={startRun}>
            ...
            <div className="card" style={{ display: "flex", gap: 16, alignItems: "center" }}>
              ...
              <button
                ...
                disabled={submitting || !preflight.ready}
              >
    ...
    <div className="modal-backdrop">
      <div className="modal">
        ...
        <p className="hint">
          ...
          {consent.declared_capabilities.length > 0 ? (
            consent.declared_capabilities.map((c) => (
