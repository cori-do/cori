...
- **Third-party libraries cannot be imported by workflow modules.** `cori check` freezes the module graph and rejects every specifier that isn't a relative file inside the workflow folder, `@cori-do/sdk`, or the pinned `zod` alias — bare names, `npm:<pkg>`, `jsr:`, and `https://esm.sh/` all fail validation, in step files and test files alike. If a step genuinely needs a library, move that work into a `cli` step (the binary owns its own dependencies) or reduce it to a pure transform over the globals above.

### Testing `code` steps (with Deno, not Node)

`code` steps are the easiest to test. Always generate a `tests/<step>.test.ts` alongside, using a captured fixture from the conversation. Write tests as `Deno.test(...)` asserting with the zero-dependency `tests/assert.ts` helper from the skill's Step 5 — never `jsr:@std/assert` or any other registry import: `cori check`'s frozen-import sweep covers test files too and rejects every non-relative specifier except `@cori-do/sdk` and `zod`. Run tests with `deno task test` (defined in the workflow `deno.json`). **Do not use Node/vitest/`package.json`.** Testing under Deno means tests resolve imports with the *same import map and the same allow-net allowlist as the runtime* — so a passing test proves the step's imports actually load under `cori run`, and a bad bare import fails the test with the identical error the runtime would raise. A Node/vitest test would resolve `lodash` from `node_modules` and pass, masking a step that breaks at runtime. See the skill's Step 5 for the `deno.json` template and `references/example_workflow.md` for a full test file.
