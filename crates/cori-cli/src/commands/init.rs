...
//!
//! Scaffolds a new Cori AI Database Proxy project by introspecting an existing
...
pub async fn run(database_url: &str, project: &str, force: bool) -> Result<()> {
    ...
    println!("   5. Start the proxy: cori serve --config cori.yaml");
...
# PROXY SETTINGS
# =============================================================================
# Cori's Postgres-compatible proxy that AI agents connect to.
...
# TENANT ISOLATION
# =============================================================================
# Row-Level Security configuration for multi-tenant isolation.
...
# VIRTUAL SCHEMA
# =============================================================================
# Control what AI agents see in information_schema queries.
...
mcp:
  enabled: true
  transport: stdio
  # http_port: 3000  # if transport: http
...
A Cori AI Database Proxy project for secure, tenant-isolated database access.
...
enforcing tenant isolation and role-based access control.
...
This starts:
- PostgreSQL-compatible proxy on port **5433**
...
### 5. Connect via AI Agent

AI agents connect using the Biscuit token as the password:

```python
import psycopg2

# Read the token
with open("tokens/acme_support.token", "r") as f:
    token = f.read().strip()

# Connect through Cori proxy
...
For AI agents that support MCP, Cori exposes typed database actions as tools.
...
| Command | Description |
|---------|-------------|
| `cori serve` | Start the proxy server |
...
| `cori proxy explain` | Show RLS injection for a query |
...
5. **Enable TLS in production** - Configure in `cori.yaml`
